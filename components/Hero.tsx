...
const Hero: React.FC = () => {
  const { heroImages, appConfig } = useContent(); 
  ...
  return (
    <div className="relative overflow-hidden bg-[#0B0F19] pt-12 pb-24 md:pt-20 md:pb-40 min-h-[90vh] flex items-center">
      ...
      {/* Background Gradients - Strictly Behind */}
      <div className="absolute inset-0 pointer-events-none z-0">
          <div 
            className={`absolute top-0 right-0 -translate-y-1/4 translate-x-1/4 w-[300px] md:w-[600px] h-[300px] md:h-[600px] bg-gradient-to-br from-${theme}-600 to-purple-600 rounded-full blur-[80px] md:blur-[120px] opacity-40 transition-transform duration-300 ease-out will-change-transform`}
            style={{ transform: `translate(${mousePos.x * -2}px, ${mousePos.y * -2}px)` }}
          />
          <div 
            className="absolute bottom-0 left-0 translate-y-1/4 -translate-x-1/4 w-[300px] md:w-[600px] h-[300px] md:h-[600px] bg-gradient-to-tr from-orange-500 to-pink-500 rounded-full blur-[80px] md:blur-[120px] opacity-30 transition-transform duration-300 ease-out will-change-transform"
            style={{ transform: `translate(${mousePos.x * 2}px, ${mousePos.y * 2}px)` }}
          />
      </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col-reverse lg:flex-row items-center w-full z-10">
        ...
        {/* Left Content */}
        <div className="w-full lg:w-1/2 text-center lg:text-left space-y-6 md:space-y-8 mt-12 lg:mt-0 relative z-20">
          <div className="animate-entry delay-100 inline-flex items-center space-x-2 bg-white/5 border border-white/10 backdrop-blur-md rounded-full px-4 py-1.5 md:px-5 md:py-2 mb-2">
            <Star className="w-3 h-3 md:w-4 md:h-4 text-orange-400 fill-orange-400 animate-spin-slow" />
            <span className={`text-${theme}-200 text-[10px] md:text-xs font-bold tracking-widest uppercase`}>Trusted by 500+ Clients</span>
          </div>
          
          <h1 className="animate-entry delay-200 text-4xl sm:text-5xl md:text-7xl font-black text-white tracking-tight leading-[1.15]" dangerouslySetInnerHTML={{ __html: appConfig.heroTitle }}>
          </h1>
          ...
          {/* Action Buttons */}
          <div className="animate-entry delay-400 flex flex-col sm:flex-row items-center justify-center lg:justify-start space-y-3 sm:space-y-0 sm:space-x-4 pt-4 px-4 sm:px-0 relative z-[200] pointer-events-auto">
            <button
              onClick={() => navigate('/contact')}
              className={`w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-${theme}-600 to-purple-600 hover:from-${theme}-500 hover:to-purple-500 text-white rounded-2xl font-bold shadow-xl shadow-${theme}-600/30 transition-all transform hover:-translate-y-1 active:scale-95 flex items-center justify-center space-x-2 text-base md:text-lg cursor-pointer select-none`}
            >
            ...
            <button
              ...
              className="w-full sm:w-auto px-8 py-4 bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-2xl font-bold backdrop-blur-md transition-all active:scale-95 flex items-center justify-center text-base md:text-lg cursor-pointer select-none"
            >
              Explore Services
            </button>
          </div>

          <div className="animate-entry delay-400 pt-6 md:pt-10 flex flex-wrap justify-center lg:justify-start gap-4 md:gap-8 text-slate-400 text-xs md:text-sm font-semibold relative z-20">
        ...
        <div className="w-full lg:w-1/2 flex justify-center lg:justify-end relative z-0 perspective-1000 mb-8 lg:mb-0 mt-10 lg:mt-0 animate-entry delay-200 pointer-events-none">
          <div className="relative w-64 h-64 sm:w-80 sm:h-80 md:w-[450px] md:h-[450px] transition-all duration-500">
             ...
             <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-4 md:-translate-y-6 bg-slate-800 p-2 md:p-4 rounded-2xl border border-slate-700 shadow-xl animate-float z-20" style={{animationDelay: '0s'}}>
                <Shield className={`w-5 h-5 md:w-8 md:h-8 text-${theme}-400`} />
             </div>
